...
export const getDestinations = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const response = await axios.get(
      `${GOOGLE_API_BASE_URL}?query=${encodeURIComponent(query)}&types=locality&key=${GOOGLE_API_KEY}`
    );

    const destinations: Destination[] = response.data.results
      .map((place: any) => {
      ...
      .filter((destination: any) => !!destination);
