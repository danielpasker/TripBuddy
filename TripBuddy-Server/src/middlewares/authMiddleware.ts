...
export const authMiddleware = (request: RequestWithUserId, response: Response, next: NextFunction) => {
  ...
  if (!token) {
    ...
    verify(token, Env.JWT_TOKEN_SECRET, (error, payload) => {
      if (error) {
        return sendError(response, StatusCodes.UNAUTHORIZED, 'Invalid token', JSON.stringify(error));
      } else {
